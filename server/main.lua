...
HandlePriceChance = function()
    ...
    -- Buzzee def didn't make this
    table.insert(Crypto.History[coin], {PreviousWorth = prevValue, NewWorth = currentValue})
    Crypto.Worth[coin] = currentValue

    QBCore.Functions.ExecuteSql(false, "UPDATE `crypto` SET `worth` = '"..currentValue.."', `history` = '"..json.encode(Crypto.History[coin]).."' WHERE `crypto` = '"..coin.."'")    
    RefreshCrypto()
end

RefreshCrypto = function()
    QBCore.Functions.ExecuteSql(true, "SELECT * FROM `crypto` WHERE `crypto` = '"..coin.."'", function(result)
